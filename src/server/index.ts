...
app.post("/api/image", async (req, res, next) => {
	const artworkID = req.body.id.length
		? req.body.id
		: "516dfb9ab31e2b2270000c45";
	...
	const getImages = async (token: string) => {
		const res = await axios.get<Response>(
			`https://api.artsy.net/api/artworks?similar_to_artwork_id=${artworkID}`,
			{
				headers: {
					"X-Xapp-Token": token,
	...
	const randomInt = Math.floor(Math.random() * data.artworks.length);
	...
	// More data, title, artist, etc
	return res.send({ url: url, id: id });
});
